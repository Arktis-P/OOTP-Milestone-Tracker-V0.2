...
# Phase 16. 패키징 및 E2E 승인 검증 (Final Release Packaging)

## [x] 16.1 E2E 시스템 통합 테스트

전체 사용자 여정(세이브 지정 → Baseline → Boxscore → 마일스톤 → 예측 → 연속기록 → 수동입력 → 격리검증)을 자동화된 단일 테스트 케이스로 검증한다.

## [x] 16.2 PyInstaller 배포 빌드 스크립트 (`build.py`)

독립 실행형 Windows 실행 파일(.exe) 생성을 위한 PyInstaller 빌드 명령 및 리소스 번들링 스크립트를 완비한다.
