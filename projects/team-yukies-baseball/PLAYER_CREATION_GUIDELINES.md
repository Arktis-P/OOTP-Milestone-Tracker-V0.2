...
`PLAYER_RATINGS.csv`에 수록하는 팀 윳키즈 선수는 사용자가 별도 역할을 지정하지 않으면 **MLB 기준 레귤러 이상의 전력**을 기본 전제로 한다.

- 야수는 기본적으로 everyday regular, 즉 주전 선수를 전제로 한다.
- 투수는 별도 지시가 없고 선발 역할 수행이 가능한 유형이면 MLB 선발 로테이션의 regular starter를 기본으로 본다.
- 유틸리티, 플래툰, 대수비, 대주자, 벤치 요원, 롱릴리프, 미들릴리프, 하이 레버리지 불펜, 셋업, 마무리 등 별도 역할은 사용자가 지정하거나 캐릭터 해석상 명확히 더 자연스러운 경우에 적용한다.
- 레귤러 기준은 모든 개별 도구를 50 이상으로 맞춘다는 뜻이 아니다. 실제 선수처럼 분명한 약점을 허용하되, 최종 역할 적합성과 `overall`은 원칙적으로 50 이상을 목표로 한다.
- 자연스러운 캐릭터 해석이 레귤러 기준에 크게 못 미치면 근거 없는 능력치 인플레이션으로 맞추지 않는다. 포지션·역할 적합성을 다시 검토하고, 그래도 맞지 않으면 확정 전에 그 차이를 보고한다.
- 레귤러는 스타와 동일하지 않다. 65+, 70+, 75+의 상위 레이팅 제한은 그대로 적용한다.
